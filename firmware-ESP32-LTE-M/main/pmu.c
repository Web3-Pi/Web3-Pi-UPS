...
/*
 * Board power architecture. The W3P MODEM V1 (M.2) card has NO AXP2101 PMU:
 * the SIM7080G VBAT rail (PP3800_SYS, 3.8 V) is generated by an always-on
 * TPS63020 buck-boost whose EN is tied to PP3300_SYS, and the ESP32 runs on
 * PP3300 directly — there is nothing for software to power-sequence, so
 * pmu_init() is a no-op here. The AXP2101 driver below is retained behind
 * BOARD_HAS_AXP2101 for the LilyGo T-SIM7080G-S3 bring-up board, where the
 * modem rails ARE PMU-gated. NB: on M.2, the old I²C SCL pin (GPIO7) is now
 * ESP_UART2_TXD, so the AXP path must NOT run there.
 */
...
#else  /* !BOARD_HAS_AXP2101 — W3P MODEM V1 (M.2): no PMU, rails are hardware-on */

esp_err_t pmu_init(void)
{
    ESP_LOGI(PMU_TAG,
             "no PMU on this board (M.2): modem VBAT = PP3800_SYS via always-on "
             "TPS63020, ESP on PP3300 — nothing to power-sequence");
    return ESP_OK;
}

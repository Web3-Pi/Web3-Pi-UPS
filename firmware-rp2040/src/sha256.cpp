/*
 * Minimal SHA-256 (FIPS 180-4). See sha256.h for provenance/licensing.
 */
#include "sha256.h"
...
static void sha256_transform(fw_sha256_ctx *ctx, const uint8_t data[64])
{
    uint32_t m[64];
...
void fw_sha256_final(fw_sha256_ctx *ctx, uint8_t out[32])
{
    ...
    /* Pad: 0x80, zeros, then the 64-bit big-endian message bit length. */
    ctx->data[i++] = 0x80;
    if (i > 56) {
